...
### 데이터셋 정제 파이프라인별 규모 (Dataset Metrics)
정성적 검수와 시계열 분리 가공을 통해, 단순 대화 나열이 아닌 '고품질 멀티턴 세션 데이터셋'을 구축했습니다.

| 데이터 가공 단계 | 데이터 규모 (수량) | 주요 처리 내용 |
| :--- | :--- | :--- |
| **01. 카카오톡 대화 원본** | 약 2만 개 행 (Row) | 카카오톡 텍스트 내보내기 순수 원본 데이터 |
| **02. Excel 1차 정성 필터링** | 약 1.7만 개 행 (Row) | 사적 내용 및 맥락 없는 일방적 노이즈 행 제거 |
| **03. 정규식 텍스트 클렌징** | 약 1만 개 행 (Row) | 개인정보, 시스템 메시지, 사진/동영상/이모티콘 태그 일괄 제거 |
| **04. 최종 학습 데이터셋** | **총 327개 대화 세션** | **4시간 공백 기준** 분리 완료된 독립 멀티턴(Multi-turn) 데이터 |

**총 대화 Turn 수** : 7539 Turn <br>
**평균 세션 길이** : 23 Turn <br>
**최대 세션 길이** : 271 Turn
...
인사이트: repetition_penalty=1.15 도입 후 정성 평가 결과, 반복 생성 현상이 감소하고 일관된 말투 유지가 관찰되었습니다.
...
### 1) 의존성 라이브러리 설치
```Bash
git clone https://github.com/gyumin04/My-Kakaotalk-LLM-Finetuning.git
